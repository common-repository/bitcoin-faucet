...
class FaucetFly
{
    protected $api_key;
    protected $currency;
    protected $api_base = 'https://www.faucetfly.com/api/v1/';
    ...
    public function send($to, $amount, $userip, $referral = "false") {
        $r = $this->__exec("send", array("to" => $to, "amount" => $amount, "referral" => $referral, "ip_address" => $userip));
        if (is_array($r) && array_key_exists("status", $r) && $r["status"] == 200) {
            return array(
                'success' => true,
                'message' => 'Payment sent to you using FaucetFly.com',
                'html' => '<div class="alert alert-success">' . htmlspecialchars($amount) . ' satoshi was sent to you <a target="_blank" href="https://www.faucetfly.com/check/' . rawurlencode($to) . '">on FaucetFly.com</a>.</div>',
                'html_coin' => '<div class="alert alert-success">' . htmlspecialchars(rtrim(rtrim(sprintf("%.8f", $amount/100000000), '0'), '.')) . ' '.$this->currency.' was sent to you <a target="_blank" href="https://www.faucetfly.com/check/' . rawurlencode($to) . '">on FaucetFly.com</a>.</div>',
                'balance' => $r['balance'],

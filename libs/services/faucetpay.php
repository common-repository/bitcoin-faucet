...
//	__execWP - TROF - TODO $response = wp_remote_post( $url, $params );
    public function __execWP($url, $params = array()) {
		$params = array_merge($params, array("api_key" => $this->api_key, "currency" => $this->currency));
        $opts = array(
            "http" => array(
                "method" => "POST",
        ...
        $fp = fopen($url, 'rb', null, $ctx);
		stream_set_timeout($fp, 15); //as of 6 May 2019 cryptoo.me is very slow
        $response = stream_get_contents($fp);
        if($response && !$this->options['disable_curl']) {
            $this->curl_warning = true;
        }
...
//		$response = $this->__execWP($url, $params);
    ...
    public function send($to, $amount, $userip, $referral = "false") {
        $r = $this->__exec("send", array("to" => $to, "amount" => $amount, "referral" => $referral, "ip" => $userip));
        if (is_array($r) && array_key_exists("status", $r) && $r["status"] == 200) {
            return array(
                'success' => true,
				'amount' => $amount,
                'message' => 'Payment sent to you using Cryptoo.me',
                'html' => '<div class="alert alert-success">' . htmlspecialchars($amount) . ' satoshi was sent to you <a target="_blank" href="https://faucetsystem.com/check/' . rawurlencode($to) . '">on FaucetSystem.com</a>.</div>',
                'html_coin' => '<div class="alert alert-success">' . htmlspecialchars(rtrim(rtrim(sprintf("%.8f", $amount/100000000), '0'), '.')) . ' '.$this->currency.' was sent to you <a target="_blank" href="https://faucetsystem.com/check/' . rawurlencode($to) . '">on FaucetSystem.com</a>.</div>',
                'balance' => $r["balance"],

...
$trof_plugin_url = plugin_dir_url( __FILE__ );
$options = <<<OPTIONS
...
$(function() {
    function load_palette(){
        ...
        if(palette == 'default'){
            ...
            var p1 = '$trof_plugin_url' + 'palettes/';
            var p2 = '.css';
            href = p1 + palette + p2;
        }
...
<div class="form-group">
    ...
    <div>
		<em>You can enter HTML,JS and CSS here as well.</em><br>
		<em>Extra code is separated from Ad boxes just for your convenience</em>
	</div>
    <br>
    <:: boxes ::>
</div>
...
TEMPLATE;
    ...
    function pretty_box_name($box){
		...
		if(strpos($ret,' NOBOX') === false) //space - because we repalced '_' -> ' '
		{
			$ret .= ' box';
		}
		else
		{
			$ret = str_replace(' NOBOX','',$ret);
		}
		return $ret;
    }

    preg_match_all('/\$data\[([\'"])(custom_(?:(?!\1).)*)\1\]/', file_get_contents(dirname( __FILE__ ) . "/../$template/index.php"), $matches);
    $boxes = '';
    $pickers = '';
